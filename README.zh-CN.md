...
`transcribe-media` 是一个面向 Agent 的媒体转写 Skill。给它一个公开视频链接或本地文件，它会优先提取页面已有字幕；没有可用字幕时，再自动获取音频并使用 AudioFlow ASR 转写。
...
如果页面没有可靠字幕，Skill 会先说明外部处理路径并征得同意。只有获得明确同意后，才会获取兼容的公开媒体音轨并交给 AudioFlow ASR 转写。小宇宙公开单集也遵守同样的授权门槛。
...
- **上传前明确同意**：只有用户同意把媒体发送到 AudioFlow API 和签名 HTTPS 存储地址，并为本次命令提供 `--allow-remote-asr` 后，媒体才会离开设备。
- **转写完成即删除**：转写进入终态后，AudioFlow 会先删除已上传的媒体，再向客户端返回终态结果。如果即时的尽力删除被中断，私有存储桶的强制生命周期策略会在 2–3 天内删除该对象。
...
- **敏感信息不进入子进程**：调用媒体解析组件时会从环境中移除 `AUDIOFLOW_TOKEN`。
...
提取网页已有字幕不需要 AudioFlow Token。只有本地媒体或缺少可用字幕的公开链接需要 ASR。使用 `--allow-remote-asr` 前，用户必须同意通过 HTTPS 把媒体和基本文件元数据发送到 `asr.audioflow123.com` 以及签名存储地址。转写进入终态后，AudioFlow 会先删除已上传的媒体，再向客户端返回终态结果。删除操作是幂等的尽力清理；如果即时删除被中断，私有存储桶的强制生命周期策略会在 2–3 天内删除遗留对象。
...
也可以通过环境变量提供 `AUDIOFLOW_TOKEN`。环境变量优先使用，且不会由 Skill 持久化保存。不要把 Token 放进命令参数、日志、源码或 Git 仓库。
...
发现可用字幕？ ── 是 → 提取并清洗字幕 → 直接输出文字
    ...
    否
    ↓
获得明确上传同意 → 获取兼容音轨 → AudioFlow ASR → 输出文字
```
...
- AudioFlow Token：仅在需要 ASR 时使用；提取公开字幕无需 Token。
